...
namespace ATL
{
	...
	public:
		...
		int GetPixelInt( int x, int y ) const throw();
		BYTE GetPixelRed( int x, int y ) const throw();
		BYTE GetPixelGreen( int x, int y ) const throw();
		BYTE GetPixelBlue( int x, int y ) const throw();
		...
		void SetPixelInt( int x, int y, int color ) throw();
...
#if _MSC_VER >= 1300 
	__declspec(selectany) CImage::CInitGDIPlus CImage::s_initGDIPlus;
	__declspec(selectany) CImage::CDCCache CImage::s_cache;
#else 
#ifndef __VC6_ATL_IMAGE_H__ 
#error "please define __VC6_ATL_IMAGE_H__ flag in stdafx.h file's begin and following..."
#endif
	// add the following 4 lines to stdafx.cpp file 
	// 
	// #if _MSC_VER < 1300 
	...
	__declspec(selectany) const DWORD CImage::createAlphaChannel = 0x01;

	__declspec(selectany) const DWORD CImage::excludeGIF = 0x01;
	__declspec(selectany) const DWORD CImage::excludeBMP = 0x02;
	__declspec(selectany) const DWORD CImage::excludeEMF = 0x04;
	__declspec(selectany) const DWORD CImage::excludeWMF = 0x08;
	__declspec(selectany) const DWORD CImage::excludeJPEG = 0x10;
	__declspec(selectany) const DWORD CImage::excludePNG = 0x20;
	__declspec(selectany) const DWORD CImage::excludeTIFF = 0x40;
	__declspec(selectany) const DWORD CImage::excludeIcon = 0x80;
	__declspec(selectany) const DWORD CImage::excludeOther = 0x80000000;
	__declspec(selectany) const DWORD CImage::excludeDefaultLoad = 0;
	__declspec(selectany) const DWORD CImage::excludeDefaultSave = CImage::excludeIcon|CImage::excludeEMF|CImage::excludeWMF;
	__declspec(selectany) const DWORD CImage::excludeValid = 0x800000ff;
		...
		, m_hDC( image.GetDC() )
	{
		if( m_hDC == NULL )
		{
			ATLASSERT(FALSE); // AtlThrow( E_OUTOFMEMORY );
		}
	}
	...
	{
		ATLASSERT( m_hBitmap == NULL ); // ATLASSUME( m_hBitmap == NULL );
...
#pragma warning(pop)
	...
	inline HBITMAP CImage::Detach() throw()
	{
		...
		ATLASSERT( m_hBitmap != NULL ); // ATLASSUME( m_hBitmap != NULL );
		ATLASSERT( m_hDC == NULL ); // ATLASSUME( m_hDC == NULL );
	...
	inline int CImage::GetPixelInt( int x, int y ) const throw()
	{
		ATLASSERT( m_hBitmap != NULL );
		ATLASSERT( (x >= 0) && (x < m_nWidth) );
		ATLASSERT( (y >= 0) && (y < m_nHeight) );
		...
		BYTE *byte=(BYTE *)( m_pBits )+(y*m_nPitch)+(x*m_nBPP>>3);
		
		int clr;
		clr=byte[2]<<16|byte[1]<<8|byte[0];
		return clr ;
	}
	inline BYTE CImage::GetPixelRed( int x, int y ) const throw()
	{
		ATLASSERT( m_hBitmap != NULL );
		ATLASSERT( (x >= 0) && (x < m_nWidth) );
		ATLASSERT( (y >= 0) && (y < m_nHeight) );
	...
	inline BYTE CImage::GetPixelGreen( int x, int y ) const throw()
	{
		ATLASSERT( m_hBitmap != NULL );
		ATLASSERT( (x >= 0) && (x < m_nWidth) );
		ATLASSERT( (y >= 0) && (y < m_nHeight) );
	...
	inline BYTE CImage::GetPixelBlue( int x, int y ) const throw()
	{
		ATLASSERT( m_hBitmap != NULL );
		ATLASSERT( (x >= 0) && (x < m_nWidth) );
		ATLASSERT( (y >= 0) && (y < m_nHeight) );
	...
	inline const void* CImage::GetPixelAddress( int x, int y ) const throw()
	{
		...
		return LPBYTE( m_pBits )+(y*m_nPitch)+(x*m_nBPP>>3);
	}
	...
	inline void CImage::SetPixelInt( int x, int y, int color ) throw()
	{
		ATLASSERT( m_hBitmap != NULL );
		ATLASSERT( (x >= 0) && (x < m_nWidth) );
		ATLASSERT( (y >= 0) && (y < m_nHeight) );
